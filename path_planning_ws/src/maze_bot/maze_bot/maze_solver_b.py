...
class Video_get(Node):
  ...
  def process_data(self, data):
    ...
    self.control.nav_path(self.bot_localizer.loc_car, shortest_path, self.maze_converter.img_shortest_path)
    ...
    if (type(path)!=int):
      if (self.control.path_iter==0):
        self.control.goal_pose_x = shortest_path[path_i][0]
        self.control.goal_pose_y = shortest_path[path_i][1]

    self.control.move(shortest_path,self.bot_localizer.loc_car,self.velocity,self.publisher)
